...
function mergeValueIntoOptions(options, value, isMultiple) {
  const merged = [...options];
  const hasValue = v => merged.some(o => String(o.value) === String(v));

  if (isMultiple) {
    const selectedOptions = List.isList(value) ? value.toJS() : value;
    if (!selectedOptions || !Array.isArray(selectedOptions)) return merged;

    selectedOptions.forEach(item => {
      const rawValue = item && typeof item === 'object' ? item.value : item;
      if (rawValue !== null && rawValue !== undefined && !hasValue(rawValue)) {
        merged.push({ label: String(rawValue), value: rawValue });
      }
    });
  ...
  if (value !== null && value !== undefined && value !== '' && !hasValue(value)) {
    merged.push({ label: String(value), value });
  }
...
export default class SelectControl extends React.Component {
  ...
  render() {
    ...
    const isMultiple = field.get('multiple', false);
    const allowCreate = field.get('allow_create', true);
    const isClearable = !field.get('required', true) || isMultiple;

    const baseOptions = [...fieldOptions.map(convertToOption)];
    const options = mergeValueIntoOptions(baseOptions, value, isMultiple);
