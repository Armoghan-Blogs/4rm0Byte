            ...
            field: {
                ...
                fields: [
                    ...
                    { 
                        ...
                        default: "github", // Default icon value
                    }
                ]
            }
        }
    ],
    pattern: /{{< keywordList >}}([^]*?){{< \/keywordList >}}/,
    fromBlock: function(match) {
        const keywords = match[1].split("{{< /keyword >}}").map(item => {
            const regex = /{{< keyword icon="([^"]+)" >}}([^]*?){{< \/keyword >}}/;
            const [, icon, text] = regex.exec(item);
            return { icon, text: text.trim() };
        });
        return { keywords };
    },
    toBlock: function(obj) {
        const keywordBlocks = obj.keywords.map(keyword => `{{< keyword icon="${keyword.icon}" >}}\n${keyword.text}\n{{< /keyword >}}`).join("\n");
        return `{{< keywordList >}}\n${keywordBlocks}\n{{< /keywordList >}}`;
    },
    toPreview: function(obj) {
        const keywordPreviews = obj.keywords.map(keyword => `<div class="keyword-preview">${keyword.text}</div>`).join("");
        return `<div class="keyword-list-preview">${keywordPreviews}</div>`;
    },
});
